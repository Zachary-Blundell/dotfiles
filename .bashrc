...
# Edit i3 config
alias ei3='lvim ~/.config/i3/config'
# Edit polybar config
alias epbar='vi ~/.config/polybar/config'
...
# Copy and paste with c and v
alias "copy=xclip" # copy to X clipboard (register *)
alias "scopy=xclip -selection clipboard" # copy to system wide clipboard (register +)
alias "v=xclip -o" # output copied content (paste)
alias "sv=xclip -o -selection clipboard" # paste from system wide clipboard (equivalent to `v -selection clipboard`)
...
# Update and upgrade
alias update='echo -e "\n\t update starting" && sudo apt update && echo -e "\n\t upgrade starting" && sudo apt upgrade && echo -e "\n\t dist-upgrade starting" && sudo apt dist-upgrade'
...
export PGUSER=postgres # makes it possible to jsut use psql to access postgresql
